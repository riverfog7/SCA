...
class Qwen3OmniMoeWithProperForward(Qwen3OmniMoeForConditionalGeneration):
    ...
    def _align_codebook_dim(self, codes: torch.Tensor) -> torch.Tensor:
        ...
        if current == target_quantizers:
            return codes
        if current < target_quantizers:
            raise ValueError(f"Mimi codes have {current} quantizers but model expects {target_quantizers}.")
    ...
    def _build_talker_prefix_single(
        ...
        thinker_hidden = thinker_outputs.hidden_states[accept_layer][batch_idx:batch_idx+1].to(self.device)

        # Get im_start positions for this sample
        sample_input_ids = input_ids[batch_idx]
        im_start_positions = torch.nonzero(sample_input_ids == config.im_start_token_id).view(-1)
        im_start_indexes = torch.cat(
            (
                im_start_positions,
                torch.tensor([input_ids.shape[1]], device=self.device, dtype=input_ids.dtype),
            ),
            dim=0,
        )
        ...
        sample_multimodal_mask = (
                (input_ids[batch_idx:batch_idx+1] == config.thinker_config.audio_token_id)
                | (input_ids[batch_idx:batch_idx+1] == config.thinker_config.image_token_id)
                | (input_ids[batch_idx:batch_idx+1] == config.thinker_config.video_token_id)
        ).to(self.device)
        ...
        if trailing_text_hidden is None:
            raise RuntimeError("Failed to build trailing_text_hidden for talker training.")

        talker_input_embed = torch.cat([embed.to(self.device) for embed in talker_input_embeds], dim=1)
        talker_input_id = torch.cat([ids.to(self.device) for ids in talker_input_ids], dim=1)
        return talker_input_embed, talker_input_id, trailing_text_hidden.to(self.device), tts_pad_embed
    ...
    def _build_talker_prefix(
        ...
        if batch_size == 1:
            ...
            return talker_embed, talker_id, trailing_hidden, tts_pad, original_prefix_lens, original_trailing_lens
        ...
        original_trailing_lens = torch.tensor(original_trailing_lens, device=self.device)
        ...
        for idx in range(batch_size):
            ...
            if embed.shape[1] < max_prefix_len:
                ...
                embed_pad = torch.zeros(1, pad_len, embed.shape[2], dtype=embed.dtype, device=embed.device)
            ...
            if ids.shape[1] < max_prefix_len:
                ...
                # Pad with pad token
                ids_pad = torch.full((1, pad_len), self.config.tts_pad_token_id, dtype=ids.dtype, device=ids.device)
            ...
            if trailing.shape[1] < max_trailing_len:
                ...
                trailing_pad = torch.zeros(1, pad_len, trailing.shape[2], dtype=trailing.dtype, device=trailing.device)
        ...
        return talker_input_embed, talker_input_id, trailing_text_hidden, tts_pad_embed, original_prefix_lens, original_trailing_lens
    ...
    def forward(self, **kwargs):
        ...
        thinker_loss = thinker_outputs.loss if hasattr(thinker_outputs, "loss") else None
        if "labels" in kwargs and (thinker_loss is not None):
            ...
            pass
             ...
             raise ValueError("Labels provided but thinker did not return loss, or vice versa.")
        ...
        if target_codes is None:
            ...
            layer0_codes = target_codes[:, 0, :]  # [batch, num_tokens] (select layer 0 code)
            num_codec_tokens = layer0_codes.shape[1]
            
            # Assertions for shape validation
            batch_size = target_codes.shape[0]
            assert target_codes.ndim == 3, f"Expected target_codes to be 3D [batch, layers, tokens], got {target_codes.shape}"
            assert target_codes.shape[1] == 16, f"Expected 16 codec layers, got {target_codes.shape[1]}"
            assert layer0_codes.shape == (batch_size, num_codec_tokens), f"layer0_codes shape mismatch: {layer0_codes.shape}"
            ...
            assert all_layer_embeds_sum.shape == (batch_size, num_codec_tokens, layer0_embeddings.shape[2]), \
            ...
            for batch_idx in range(batch_size):
                ...
                sample_trailing_hidden = trailing_text_hidden[batch_idx:batch_idx+1]  # [1, max_trailing_len, hidden]
                sample_all_layer_sum = all_layer_embeds_sum[batch_idx:batch_idx+1]  # [1, num_tokens, hidden]
                ...
                for pos in range(num_codec_tokens):
                    ...
                    if prev_pos < sample_trailing_len:
                        text_hidden = sample_trailing_hidden[:, prev_pos:prev_pos + 1, :]
                    else:
                        text_hidden = tts_pad_embed

                    pos_embed = sample_all_layer_sum[:, prev_pos:prev_pos + 1, :] + text_hidden
                    codec_input_embeds_list.append(pos_embed)

                # EOS token
                last_pos = num_codec_tokens - 1
                if last_pos < sample_trailing_len:
                    eos_text_hidden = sample_trailing_hidden[:, last_pos:last_pos + 1, :]
                else:
                    eos_text_hidden = tts_pad_embed
                eos_input_embed = sample_all_layer_sum[:, last_pos:last_pos + 1, :] + eos_text_hidden
            ...
            if batch_codec_inputs:
                codec_input_embeds = torch.cat(batch_codec_inputs, dim=0).to(self.talker.dtype)
                assert codec_input_embeds.shape == (batch_size, num_codec_tokens, layer0_embeddings.shape[2]), \
            ...
            assert full_inputs_embeds.ndim == 3, f"full_inputs_embeds should be 3D, got {full_inputs_embeds.shape}"
            ...
            for batch_idx in range(batch_size):
                ...
                prefix_mask = torch.full((1, prefix_mask_len), -100, dtype=torch.long, device=self.device)
                ...
                gap_mask = torch.full((1, gap_len), -100, dtype=torch.long, device=self.device)
                
                # 4. Labels for code[1:] + EOS: at [max_prefix ... end]
                # layer0_codes[1:] + EOS
                label_code_rest = layer0_codes[batch_idx:batch_idx+1, 1:] # [1, N-1]
                label_eos = torch.full((1, 1), codec_eos_id, dtype=torch.long, device=self.device)
                ...
                sample_labels = torch.cat([prefix_mask, label_code0, gap_mask, label_code_rest, label_eos], dim=1)
            ...
            attention_mask = torch.zeros((batch_size, seq_len), dtype=torch.long, device=self.device)
            for batch_idx in range(batch_size):
                sample_prefix_len = original_prefix_lens[batch_idx].item()
            ...
            if not self.config.train_mtp:
                ...
                for batch_idx in range(batch_size):
                    ...
                    # Shape: [1, 1, hidden]
                    h0 = talker_hidden[batch_idx:batch_idx+1, sample_prefix_len-1:sample_prefix_len, :]
                    ...
                    h_rest = talker_hidden[batch_idx:batch_idx+1, prefix_len : prefix_len + num_codec_tokens - 1, :]
                ...
                assert codec_hidden.shape == (batch_size, num_codec_tokens, talker_hidden.shape[2]), \
                ...
                for mtp_layer_idx in range(num_mtp_layers):
                    ...
                    target_layer_codes = target_codes[:, mtp_layer_idx + 1, :].to(self.device)
                    # Assert target shape
